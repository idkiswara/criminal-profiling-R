...
for(index in min(berita$IDBERITA):length(berita$ISIBERITA)){
  ...
  if(length(pecahberita) != 0){
    ...
    query <- sprintf("insert into detailberita values('','%s','%s',%d,%s,'%s',%s)", copyberita[index,6], copyberita[index,4], berita$IDBERITA[index], copyberita[index,5], copyberita[index,2], copyberita[index,3])
    ...
    if(is.null(carinama) == FALSE){
      for(qwe in 1:length(cariindex)){
        ...
        if (length(umur) != 0){
          if (nchar(umur[qwe]) < 3) {
            query <- sprintf("INSERT INTO keterlibatan VALUES ('',%d,'%s',%d,'N/A')",berita$IDBERITA[index],cariindex[qwe],umur[qwe])
            if(length(query) != 0){
              sql <- dbSendQuery(con, query)
            }
          }
    ...
    if(length(cariindex) !=0){
      ...
      for(counter in 1:length(cariindex)){
        if(length(which(cariindex[counter] == daftarnama$V1)) != 0){
          ...
          query <- sprintf("UPDATE keterlibatan SET jeniskelamin = '%s' WHERE namaorang = '%s' AND iddetail=%d", kelamin[hitung], cariindex[counter], berita$IDBERITA[index])
          sql <- dbSendQuery(con,query)
  ...
  query <- sprintf("UPDATE berita SET idstatus=1 where idberita=%d",berita$IDBERITA[index])
  sql <- dbSendQuery(con, query)
}
